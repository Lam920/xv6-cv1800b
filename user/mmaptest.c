...
#ifdef DEBUG_MMAP
static void get_file_stat(int fd, struct stat *st) {
  if (fstat(fd, st) < 0) {
    printf("mmaptest: fstat failed\n");
    exit(1);
  }
  printf("file size: %ld\n", st->size);
  printf("file type: %d\n", st->type);
...
mmap_test(void)
{
  ...
  int num_bytes;
  if((num_bytes = read(fd, buf, PGSIZE)) != PGSIZE/2)
  {
    printf("dirty read #2 read %d bytes\n", num_bytes);
    err("dirty read #2");
  }
  for (i = 0; i < PGSIZE/2; i++){
    if (buf[i] != 'C')
