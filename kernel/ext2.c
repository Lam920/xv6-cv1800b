...
{
  printf("Reading ext2 superblock from device %d\n", dev);
  struct buf *bp;
  ...
  printf("Ext2 block groups count: %d\n", (int)(es->s_blocks_count / es->s_blocks_per_group) + 1);
  printf("Ext2 block size: %d\n", 1024 << es->s_log_block_size);
  printf("Ext2 block/group: %d and inodes/group: %d\n",
          es->s_blocks_per_group, es->s_inodes_per_group);
  ...
  printf("Inode table blocks per group: %d\n", sbi->s_itb_per_group);
  printf("Group descriptors per block: %d\n", sbi->s_desc_per_block);
  printf("----------------------------------------------------------\n");
                                 ...
                                 Indirect chain[4])
{
  ...
  {
#ifdef DEBUG_EXT2
    printf("ext2_get_branch: direct block not exist\n");
#endif
    goto no_block;
  }
  while (--depth) {
...
#ifdef DEBUG_EXT2
  printf("ext2_find_near: inode %d block group %d starts at block %d\n",
         inode->inum, i_block_group, (int)bg_start);
#endif
  colour = (myproc()->pid % 16) *
...
repeat:
  ...
  if (ext2_set_bit_atomic(grp_goal,
     ...
     */
#ifdef DEBUG_EXT2
    printf("ext2_try_to_allocate: block %d was busy, bitmap value after: %d\n", 
           grp_goal, ext2_test_bit(grp_goal, (unsigned long *)bitmap_bh->data));

    printf("ext2_try_to_allocate: block %d was busy\n", grp_goal);
#endif
    start++;
    grp_goal++;
    if (start >= end)
                ...
                unsigned long *count, int *errp)
{
  ...
  *errp = -1;
  ...
  printf("ext2_new_blocks: group_no=%d\n", group_no);
#endif
retry_alloc:
...
#ifdef DEBUG_EXT2
    printf("ext2_new_blocks: group_no=%d, free_blocks=%lu, grp_alloc_blk=%d and num: %d\n",
           group_no, free_blocks, grp_alloc_blk, num);
#endif
    if (grp_alloc_blk >= 0)
      goto allocated;
  }
...
#ifdef DEBUG_EXT2
  printf("ext2_alloc_blocks: target=%d\n", target);
#endif
...
#ifdef DEBUG_EXT2
  printf("ext2_alloc_branch: indirect_blks=%d, blks=%d, goal=%d\n",
         indirect_blks, *blks, (int)goal);
#endif
...
#ifdef DEBUG
  printf("ext2_bmap: depth=%d, blocks_to_boundary=%d\n",
         depth, blocks_to_boundary);
  printf("ext2_bmap: offsets: ");
  for (int i = 0; i < depth; i++)
    printf("%d ", offsets[i]);
  printf("\n"); 
  printf("ext2_bmap: partial key=%d\n",
         partial ? (int)partial->key : -1);
#endif
...
#ifdef DEBUG
  printf("ext2_bmap: goal=%d\n", (int)goal);
  printf("ext2_bmap: indirect_blks=%d\n", indirect_blks);
#endif
  /*
...
int
ext2_writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;
  ...
  for (tot = 0; tot < n; tot += m, off += m, src += m) {
    ...
    if(either_copyin(bp->data + (off % sb[ip->dev].blocksize), user_src, src, m) == -1) {
      ext2_ops.brelse(bp);
      break;
    }
    ext2_ops.bwrite(bp);
...
/**
 * Its is called because the itable lookup failed
 */
int
