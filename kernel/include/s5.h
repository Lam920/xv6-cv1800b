...
int            s5_readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n);
int            s5_writei(struct inode *ip, int user_src, uint64 src, uint off, uint n);
...
int            s5_dummy(void); // To avoid empty struct
