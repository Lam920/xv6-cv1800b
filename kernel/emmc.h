...
#define SET_BUS_WIDTH           (6 | IS_APP_CMD) // ACMD6
...
struct emmc {
    ...
    uint32_t    card_ocr;           // OCR register value from CMD58/ACMD41
    uint32_t    card_rca;           // Relative Card Address (from CMD3)
    uint32_t    last_interrupt;     /* 最新の割り込み */
    uint32_t    last_error;         /* 最新のエラー */

    struct tscr scr;                // SCR info from ACMD51
    ...
    uint32_t    last_r0;             // Last response registers [0]
