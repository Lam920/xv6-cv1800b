...
/*
List of supported filesystems by OS
It is used by the mount syscall to find the filesystem_type
of the filesystem to be mounted
*/
struct {
  struct spinlock lock;
...
/* Init spinlock for vfsmlist and vfspool
    + vfspool is the pool of vfs entries: fixed size array
    + vfsmlist is the list of mounted filesystems ~ "instances" of filesystems running
      on a (major, minor) device
*/
void
initvfsmlist(void)
{
...
/* Register a new VFS to xv6 kernel */
int
...
/* Get filesystem type based on its name */
struct filesystem_type*
getfs(const char *fs_name)
{
  printf("getfs: searching for %s\n", fs_name);
  if (fs_name == 0) {
    printf("getfs: fs_name is null");
    return 0;
  }
...
/* Allocate a vfs entry from the pool */
struct vfs*
allocvfs()
{
  struct vfs *vfs;
...
/*
  ...
  It is used when the system boots, it will add the s5 filesystem on vfsmlist
  as the root filesystem. Other filesystems will be added when a mount syscall is made (add to vfsmlist)
*/
void
installrootfs(void)
{
  printf("Installing rootfs...\n");
  /* Alloc for a new vfs */
  if ((rootfs = allocvfs()) == 0) {
      panic("Failed on rootfs allocation");
  }

  rootfs->major = ROOTDEV;
...
void vfs_fsinit(int dev) {
  printf("vfs_fsinit: dev %d\n", dev);
  // Read the superblock of rootfs
  // It is needed because the rootfs->fs_t->ops->readsb will use
  // the dev number to read the superblock
  installrootfs();
  rootfs->fs_t->ops->readsb(dev, &sb[dev]);
}

/*
Get the vfs entry from the list of mounted filesystems
This vfs entry will be pushed by putvfsonlist when a new
filesystem is mounted on a (major, minor) device
*/
struct vfs*
...
/*
Put a new vfs entry on the list of mounted filesystems
It is used when a new filesystem is mounted on a (major, minor) device
The vfs entry is allocated from the vfspool
*/
int
...
/* 
Generic unlock function for inode
It is used by all filesystem implementations
*/
void
...
generic_readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
...
#endif
    printf("generic_readi: read device %d not supported\n", ip->major);
    return -1;
  }
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    uint addr = ip->iops->bmap(ip, off/sb[ip->dev].blocksize);
    if(addr == 0)
      break;
...
iget(uint dev, uint inum, int (*fill_inode)(struct inode *))
{
...
#ifdef DEBUG
  printf("iget: allocating new inode\n");
#endif
  // Get the filesystem type of this inode

  fs_t = getvfsentry(ROOTDEV, dev)->fs_t;
  ...
  release(&itable.lock);
  // lambt9: Fill private DINODE info to struct inode
  if (!fill_inode(ip)) {
    panic("Error on fill inode");
  }

#ifdef DEBUG
  printf("iget: allocated inode %d on dev %d\n", inum, dev);
#endif
...
namex(char *path, int nameiparent, char *name)
{
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    ir = next->fs_t->ops->getroot(ROOTDEV, next->dev);
